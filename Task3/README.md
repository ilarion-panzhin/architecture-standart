...
**СМС‑подтверждение и уведомления**: используем существующий СМС‑шлюз банка; ИБ инициирует OTP, АБС отправляет статусные уведомления.
...
**Security**: TLS 1.2+/1.3, OTP через СМС, IAM/SSO (поэтапно), RBAC/ABAC для бэк‑офиса, аудит (события входа, заявки, изменения ставок).
...
**Как**: API принимает заявки из ИБ (и опционально с сайта — «Вариант A»), Worker публикует события в шину, хранит outbox/inbox и аудит интеграций.
